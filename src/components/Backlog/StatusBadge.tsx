...
export function StatusBadge({ ticket }: { ticket: Ticket }) {
  ...
  const currentCol = columns.find(c => c.id === ticket.columnId);
  const allCols = [...columns].sort((a, b) => a.order - b.order);
      ...
      {open && (
        <>
          ...
          <div className="bl-status-menu" role="menu" style={{ top: menuPos.top, right: menuPos.right }}>
            <div className="bl-status-menu-label">Move to</div>
            {allCols.map(col => {
              const s = getBadgeStyle(col);
              const isCurrent = col.id === ticket.columnId;
              return (
                <button
                  key={col.id}
